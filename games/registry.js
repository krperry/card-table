...
// games/hearts, games/spades, games/cribbage, games/rummy) that receives a
// small `deps` object of shared table/networking primitives and returns the
// game-module interface - there are no remaining static/non-playable
// placeholder entries.
...
const createRummyGame = require('./rummy');
...
module.exports = function buildGameRegistry(deps) {
  ...
  const rummy = createRummyGame(deps);
  ...
  const definitions = {
    ...
    rummy: { type: 'rummy', name: rummy.name, playable: rummy.playable !== false, minPlayers: rummy.minPlayers, maxPlayers: rummy.maxPlayers }
  };
