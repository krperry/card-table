...
test('only the host can change the table code, and it does not affect already-joined players', async () => {
  const port = 3152;
  const child = spawn(process.execPath, ['server.js'], {
    cwd: path.join(__dirname, '..'),
    env: { ...process.env, PORT: String(port), NODE_ENV: 'test' },
  ...
  try {
    await waitForServer(child, port);
    host = await connectAndRegister(port, 'change-host-' + Date.now() + '@example.com', 'ChangeHost' + Date.now());
    guest = await connectAndRegister(port, 'change-guest-' + Date.now() + '@example.com', 'ChangeGuest' + Date.now());
    latecomer = await connectAndRegister(port, 'change-late-' + Date.now() + '@example.com', 'ChangeLate' + Date.now());
    ...
    const table = await new Promise((resolve, reject) => {
      ...
      host.emit('createTable', { name: 'Open Table', gameType: 'uno' });
    });

    assert.equal(table.hasCode, false);

    guest.emit('joinTable', { tableId: table.id });
    await waitForEvent(guest, 'tableState', (payload) => payload && payload.table && payload.table.players.length === 2);

    guest.emit('changeTableCode', { code: '1111' });
    ...
    host.emit('changeTableCode', { code: '9999' });
    const confirmation = await waitForEvent(host, 'serverMessage', (payload) => payload && payload.type === 'info');
    assert.match(confirmation.message, /table code updated/i);

    // The already-seated guest should still be able to leave normally (proving
    // they were never removed by the code change).
    const guestLeavePromise = waitForEvent(guest, 'tableState', (payload) => payload === null);
    guest.emit('leaveTable');
    await guestLeavePromise;

    latecomer.emit('joinTable', { tableId: table.id });
    const latecomerRejection = await waitForEvent(latecomer, 'serverMessage', (payload) => payload && payload.type === 'error');
    assert.match(latecomerRejection.message, /incorrect table code/i);

    latecomer.emit('joinTable', { tableId: table.id, code: '9999' });
    const latecomerJoined = await waitForEvent(latecomer, 'tableState', (payload) => payload && payload.table && payload.table.players.length === 2);
    assert.equal(latecomerJoined.table.id, table.id);
  } finally {
    if (host && host.connected) {
      host.disconnect();
    }
    ...
    if (latecomer && latecomer.connected) {
